...
export function effectiveCropName(op: {
...
export async function fetchAvailableCrops(
  ...
  const crops = Array.from(
    new Set(rows.map((d) => effectiveCropName(d)).filter((c): c is string => !!c)),
...
export async function saveAnalysisResult(
  ...
  const { error } = await (supabase.from("irrigation_analysis_results") as any).upsert(result, {
    onConflict: "user_id,jd_operation_id",
  });
...
export async function deleteAnalysisResult(userId: string, operationId: string): Promise<void> {
  await (supabase.from("irrigation_analysis_results") as any)
    .delete()
    .eq("user_id", userId)
  ...
  return operations
    ...
    .map((op) => {
      ...
      if (field.boundary_geojson) {
        const totalSqm = turfArea({
          type: "Feature",
          geometry: field.boundary_geojson as any,
          properties: {},
        });
        ...
        if (!preIrrigation && field.has_irrigated_boundary && field.irrigated_boundary_geojson) {
          ...
          try {
            ...
            const irrigFeature = {
              type: "Feature" as const,
              geometry: field.irrigated_boundary_geojson as any,
              properties: {},
            };
            const fc = {
              type: "FeatureCollection" as const,
              features: [fieldFeature, irrigFeature],
            };
            const intersectFn =
              (turfIntersect as any).intersect || (turfIntersect as any).default || turfIntersect;
            const clipped = intersectFn(fc);
            if (clipped) {
              irrigatedSqm = turfArea({
                type: "Feature",
