...
export function useFields() {
  ...
  const updateIrrigationStartYear = useCallback(
    async (fieldId: string, year: number | null) => {
      setAllFields((prev) =>
        prev.map((f) => (f.id === fieldId ? { ...f, irrigation_start_year: year } : f)),
      );
