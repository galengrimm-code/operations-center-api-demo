...
function getSortValue(row: ReportRow, key: SortKey): number | string {
  switch (key) {
    ...
    case "crop":
      return formatCropName(effectiveCropName(row.operation));
    case "irrigatedAcres":
    ...
    case "irrYield":
      return (
        toDryYield(
          ...
          effectiveCropName(row.operation),
        ) ?? -1
      );
    case "dryYield":
      return (
          ...
          {sortedRows.map((row) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3 text-right font-medium text-cyan-400">{fmt(totalBuAc)}</td>
...
function CropCell({ row }: { row: ReportRow }) {
    ...
    return (
      <div className="flex items-center gap-1.5">
        <select
          ...
          className="rounded-md border border-emerald-500/40 bg-slate-800 px-2 py-1 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-emerald-500"
        >
          {CROP_OPTIONS.map((o) => (
  ...
  return (
    <button
      ...
      className="group inline-flex items-center gap-1.5 transition-colors hover:text-emerald-300"
      title={
        isOverridden
          ? `Originally: ${formatCropName(row.operation.crop_name)} (overridden)`
          : "Click to override crop type"
      }
    >
      <span>{formatCropName(effective)}</span>
      {isOverridden && (
        <span className="text-[9px] uppercase tracking-wider text-cyan-400">override</span>
      )}
