...
export default function ProgressPage() {
  ...
  if (!orgId) {
    return (
      <div className="flex min-h-[calc(100vh-48px)] items-center justify-center bg-slate-950 p-6">
        <div className="text-center">
          <Sprout className="mx-auto mb-3 h-10 w-10 text-slate-600" />
          <p className="text-slate-400">
            Connect a John Deere organization to see season progress.
          </p>
        </div>
      </div>
    );
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-400">Planting and harvest pace, {year}</p>
          </div>
          <select
            value={year}
            onChange={(e) => setYear(Number(e.target.value))}
            className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-sm text-slate-200 focus:border-emerald-500/40 focus:outline-none"
          >
            {YEARS.map((y) => (
        ...
        return (
          <div key={c.crop} className="glass rounded-xl p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="font-mono-data text-xs text-slate-400">{percent}%</span>
            </div>
            <div className="font-mono-data mb-1 text-2xl font-semibold text-white">
            ...
            <div className="flex items-center justify-between text-xs text-slate-400">
              ...
              <span className="font-mono-data">{fmtAcres(remaining)} ac left</span>
            </div>
          </div>
        );
      })}
...
function ChartTooltip({
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-900/95 px-3 py-2 text-xs shadow-lg">
      <div className="mb-1.5 font-semibold text-slate-100">{fmtMonthDay(String(label))}</div>
      <div className="space-y-2">
        {visibleCrops.map((crop) => (
          <div key={crop}>
            <div className="mb-0.5 flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: cropColor(crop) }} />
              <span className="font-medium text-slate-200">{prettyCrop(crop)}</span>
            </div>
              ...
              {yearsIncluded.map((y) => {
                ...
                return (
                  <span key={y} className="contents">
                    ...
                    >
                      {pctVal.toFixed(0)}%{" "}
                      <span className="opacity-60">({fmtAcres(acVal)} ac)</span>
                    </span>
                  </span>
                );
              })}
...
function CumulativeChart({ data }: { data: SeasonProgress }) {
  if (data.cumulative.length === 0) {
    return (
      <div className="glass mb-6 rounded-xl p-8 text-center">
  ...
  return (
    <div className="glass overflow-hidden rounded-xl">
      <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-3">
        ...
        <span className="font-mono-data text-xs text-slate-500">{fields.length} fields</span>
      </div>
            ...
            <div key={row.field_id} className="px-5 py-3">
              <div className="flex items-center gap-4 text-sm">
                ...
                >
                  ...
                  />
                  <span className="truncate font-medium text-white">{row.field_name}</span>
                  {row.crop && (
                    <span className="text-xs text-slate-400">{prettyCrop(row.crop)}</span>
                  )}
                  {row.source === "manual" && (
                    <span className="font-mono-data rounded bg-emerald-500/10 px-1.5 py-0.5 text-[10px] text-emerald-400">
                      manual
                    </span>
                  )}
                </button>
                <span className="font-mono-data w-24 text-right text-xs text-slate-400">
                  {fmtDate(row.planted_date)}
                </span>
                <span className="font-mono-data w-32 text-right text-xs text-slate-300">
              ...
              {isEditing && edit && (
                <div className="mt-3 grid grid-cols-2 gap-2 pl-4 md:grid-cols-5">
                  <label className="col-span-2 text-xs text-slate-400 md:col-span-1">
                    <span className="mb-1 block">Crop</span>
                    <select
                      value={edit.intended_crop}
                      onChange={(e) => setEdit({ ...edit, intended_crop: e.target.value })}
                      className="w-full rounded-md border border-white/[0.06] bg-white/[0.03] px-2 py-1.5 text-xs text-slate-200 focus:border-emerald-500/40 focus:outline-none"
                    >
                      <option value="" className="bg-slate-900">
                        —
                      </option>
                      <option value="CORN" className="bg-slate-900">
                        Corn
                      </option>
                      <option value="SOYBEANS" className="bg-slate-900">
                        Soybeans
                      </option>
                      <option value="WHEAT" className="bg-slate-900">
                        Wheat
                      </option>
                      <option value="MILO" className="bg-slate-900">
                        Milo
                      </option>
                    </select>
                  </label>
                  <label className="text-xs text-slate-400">
                    <span className="mb-1 block">Target ac</span>
                    <input
                      type="number"
                      step="0.1"
                      value={edit.intended_acres}
                      onChange={(e) => setEdit({ ...edit, intended_acres: e.target.value })}
                      className="font-mono-data w-full rounded-md border border-white/[0.06] bg-white/[0.03] px-2 py-1.5 text-xs text-slate-200 focus:border-emerald-500/40 focus:outline-none"
                    />
                  </label>
                  <label className="text-xs text-slate-400">
                    <span className="mb-1 block">Plant date</span>
                    <input
                      type="date"
                  ...
                  <label className="text-xs text-slate-400">
                    <span className="mb-1 block">Planted ac</span>
                    <input
                      type="number"
                      step="0.1"
                  ...
                  <div className="col-span-2 flex items-end gap-2 md:col-span-1">
                    <button
                      onClick={save}
                      disabled={saving}
                      className="bg-emerald-500/15 flex flex-1 items-center justify-center gap-1 rounded-md border border-emerald-500/20 px-3 py-1.5 text-xs text-emerald-400 hover:bg-emerald-500/25 disabled:opacity-50"
                    >
                      {saving ? (
                    ...
                    <button
                      onClick={cancel}
                      disabled={saving}
                      className="rounded-md border border-white/[0.06] bg-white/[0.03] px-3 py-1.5 text-xs text-slate-400 hover:bg-white/[0.06]"
                    >
                      Cancel
                    </button>
                  </div>
